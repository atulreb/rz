...
//                }
	...
	function getServiceBlogs($service, $limit = '', $offset = '')
    {
        $where = "mbp.status='1' AND service_category = '".$service."'";
        $this->db->select("mbp.*,mu.username");
        $this->db->from(TABLES::$MST_BLOG_POSTS . ' as mbp');
        $this->db->join(TABLES::$ADMIN_USER . ' as mu', 'mu.id=mbp.posted_by', 'left');
        $this->db->where($where);
...
function getAllActiveBlogListByService($condition, $limit = '', $offset = '') {
        $this->db->select("mbp.*,mu.*");
        $this->db->from(TABLES::$MST_BLOG_POSTS . ' as mbp');
        $this->db->join(TABLES::$ADMIN_USER . ' as mu', 'mu.id=mbp.posted_by', 'left');
        $this->db->where('mbp.status', '1');
        $this->db->where('mbp.service_category', $condition);
        $this->db->order_by('mbp.posted_on', 'desc');
        if (!$limit == '') {
            $this->db->limit($limit);
        }
